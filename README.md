...
偽装転売・産地偽装・中抜き ── 流通途中で「誰が作り、誰を経由し、誰の手に渡ったか」が後追いできないと、被害が表面化した時には既に手遅れになる。単一企業の台帳を信じるのではなく、**3 社で共有された改ざん困難な台帳** に譲渡履歴を記録することで、最終販売者 C が自組織の手元から起点 A を証明できる世界を作る。これが本 PoC が扱う領域。

### ゴール

3 社（A / B / C）で共有する改ざん困難な台帳の上に、製品単位の譲渡履歴を記録し、**販売店 C の立場から起点 A を証明する** ことを実演する。最大の山場は N4 後の **C 視点クライマックス** (`demo_verify_as_c.sh`) ── C が自組織の credential で自組織 peer に履歴を問い合わせるだけで、A / B の言い分を信用せずに起点を機械判定できる瞬間。
...
| シナリオ | 期待結果 |
|---|---|
| E1: 所有者偽装（C が「B から譲り受けた」と嘘の由来を主張し横取り） | `[OWNER_MISMATCH]` で endorsement 失敗 |
...
第三者が再現する際は、まず以下で前提ソフトを揃える（検証済み）。
...
WSL2 の場合はメモリを 8GB 以上に増やす（`%USERPROFILE%\.wslconfig`、詳細は下記「よくあるエラーと対処」）。
...
C (Org3) が「直前まで B が持っていた product だ」と嘘の由来を主張し、B→C の移転を自分の credential で書き込もうとする:
...
期待:
```
Error: [OWNER_MISMATCH] fromOwner does not match currentOwner: from=Org2MSP, current=Org3MSP
```
- exit code ≠ 0
