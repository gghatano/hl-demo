...
- [ ] MSP ID は fabric-samples 標準（`Org1MSP` / `Org2MSP` / `Org3MSP`）を使用（Issue #1）
...
- [ ] `CreateProduct`: `clientIdentity.MSPID === 'Org1MSP'` 検証（メーカー A のみ）
...
- [ ] commit 時 `--signature-policy` 明示（`OR('Org1MSP.peer','Org2MSP.peer','Org3MSP.peer')`）
